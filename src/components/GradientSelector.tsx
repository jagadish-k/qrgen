...
const QRPreview: React.FC<{ gradient: Gradient; qrData: string; size: number }> = ({ gradient, qrData, size }) => {
  ...
  useEffect(() => {
    ...
    try {
      ...
      new QRious({
        ...
        value: qrData,
        size: size,
        level: 'L', // Lower error correction for better performance in preview
        foreground: '#000000',
        background: '#FFFFFF'
      })
      ...
      if (ctx && gradient.colors.length > 1) {
        ...
        // Create gradient
        const gradientStyle = ctx.createLinearGradient(0, 0, canvas.width, canvas.height)
        gradient.colors.forEach((color, index) => {
          gradientStyle.addColorStop(index / (gradient.colors.length - 1), color)
        })
  ...
  return <canvas ref={canvasRef} width={size} height={size} className="w-full h-full rounded-lg" />
}
...
export const GradientSelector: React.FC<GradientSelectorProps> = ({
  ...
  const filteredGradients = useMemo(() => {
    return gradientOptions.filter(gradient => matchesSearchTerm(gradient, searchTerm))
        ...
        <div className="max-h-80 overflow-y-auto">
          ...
          {filteredGradients.length === 0 && (
            <div className="text-center py-8">
              <p className="transition-colors duration-300" style={{color: isDarkMode ? 'rgba(156, 163, 175, 0.8)' : 'rgba(107, 114, 128, 0.8)'}}>
